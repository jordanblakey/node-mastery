...
handlers.index = function(data, callback) {
  ...
  if (data.method == 'get') {
    ...
    var templateData = {
      ...
      'head.description':
        "We offer free HTTP/HTTPS sites. When your site goes down, we'll send you a text to let you know",
      'body.class': 'index'
    }
...
handlers.accountCreate = function(data, callback) {
  if (data.method == 'get') {
    ...
    // Read in a template as a string
    helpers.getTemplate('account-create', templateData, function(err, str) {
      if (!err && str) {
        // Add the universal header and footer
        helpers.addUniversalTemplates(str, templateData, function(err, str) {
          if (!err && str) {
...
handlers._users.get = function(data, callback) {
  ...
  if (phone) {
    ...
    // Verify token valid for phone number
...
handlers._users.put = function(data, callback) {
  ...
  var phone =
    typeof data.payload.phone == 'string' && data.payload.phone.trim().length > 0 ? data.payload.phone.trim() : false
          ...
          if (!err && userData) {
            _data.delete('users', phone, function(err) {
              if (!err) {
                ...
                var userChecks =
                  typeof userData.checks == 'object' && userData.checks instanceof Array ? userData.checks : false
                    ...
                    _data.delete('checks', checkId, function(err) {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {
                            Error:
                              "Errors encountered when trying to delete the user's checks. All checks may not have been deleted successfully"
                          })
                        }
                      }
                    })
  ...
  if (id) {
    ...
    _data.read('checks', id, function(err, checkData) {
      if (!err && checkData) {
        // Get the token from the headers
        var token = typeof data.headers.token == 'string' ? data.headers.token : false
        // Verify that the given token is valid and belongs to the person that created the chek
...
// Checks - put
// Required data: id
// Optional data: protocol, url, method, successCodes, timeoutSeconds (one required)
              ...
              if (!err) {
                ...
                _data.read('users', checkData.userPhone, function(err, userData) {
                  if (!err && userData) {
                    ...
                    callback(500, {
                      Error: "Could not find user who created the check, so couldn't remove from the list of checks"
                    })
                  }
                })
              } else {
